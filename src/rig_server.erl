...
-export([init/3, handle_msg/2, terminate/2]).
...
handle_msg(?MSG_RELOAD,
           #state{reload_delay = ReloadDelay,
    ...
    {ok,
     State#state{timestamps = Timestamp2, timer_ref = new_timer(ReloadDelay, ?MSG_RELOAD)}};
handle_msg({?MSG_RELOAD_CONFIG, {Name, _, _, _} = Config}, #state{tids = Tids} = State) ->
    {Current, New, Tids2} = new_table(Name, Tids),
    async_reload(Config, Current, New),

    {ok, State#state{tids = Tids2}}.
...
%% private
async_reload(Config, Current, New) ->
    spawn(fun() -> reload(Config, Current, New) end).
...
configs_validate([{BaseDir, Configs} | T], Acc) ->
    Configs2 =
        [{Table, BaseDir ++ Filename, DecoderFun, Options}
...
reload( { Name , Filename , DecoderFun , Opts } , Current , New ) -> try Timestamp = os : timestamp( ) , { ok , File } = file : open( Filename , [ binary , read ] ) , KeyElement = ?LOOKUP( key_element , Opts , ?DEFAULT_KEY_ELEMENT ) , ok = rig_utils : read_file( File , DecoderFun , New , KeyElement ) , ok = file : close( File ) , ok = rig_index : add( Name , New ) , Subscribers = ?LOOKUP( subscribers , Opts , ?DEFAULT_SUBSCRIBERS ) , [ Pid ! { rig_index , update , Name } || Pid <- Subscribers ] , cleanup_table( Current ) , Diff = timer : now_diff( os : timestamp( ) , Timestamp ) div 1000 , error_logger : info_msg( "~p config reloaded in ~p ms" , [ Name , Diff ] ) catch ?EXCEPTION( E , R , Stacktrace ) -> error_logger : error_msg( "error loading ~p: ~p:~p~n~p~n" , [ Name , E , R , ?GET_STACK( Stacktrace ) ] ) end .
