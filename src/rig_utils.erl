...
-export([change_time/1, decode_varint/1, encode_varint/1, lookup/3, match_all/1,
         parse_fun/1, read_file/4]).

%% public
-spec change_time(file:filename()) -> pos_integer() | undefined.
change_time(File) ->
    case file:read_file_info(File, [raw, {time, posix}]) of
        {ok, #file_info{mtime = Mtime}} ->
            Mtime;
        {error, Reason} ->
            error_logger:warning_msg("can't read file ~p: ~p~n", [File, Reason]),
            undefined
    end.
...
-spec encode_varint(I :: integer()) -> binary().
encode_varint(I) ->
    encode_varint(I, []).
...
-spec match_all('$end_of_table' | {[any()], ets:continuation()}) -> [[any()]].
match_all('$end_of_table') ->
    [];
match_all({Match, Continuation}) ->
    [Match | match_all(ets:match_object(Continuation))].

-spec parse_fun(string()) -> {ok, fun()} | {error, invalid_fun}.
parse_fun(Decoder) ->
    try
...
-spec read_file(file:io_device(), decoder(), ets:tid(), pos_integer()) -> ok.
